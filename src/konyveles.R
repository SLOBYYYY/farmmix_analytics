...
((:P_UGYFELBESOROLAS = 0 or :P_UGYFELBESOROLAS = -1) or ((select first 1 ID_HIVATKOZAS
                                                          from BESOROLASERTEK
                                                          where ID_BESOROLASTETEL = :P_UGYFELBESOROLAS and
                                                          ID_HIVATKOZAS = B.ID_UGYFEL) > 0))
...
R_FIZDATUM = P_TARGYNAP;
if ((V_FORINTOSSZEG < 0) or (V_DEVIZAOSSZEG < 0)) then
begin
...
if ((R_INTERVALLUM is not null) and
    (V_ARFBIZONYLAT = 0)) then
begin
if (P_INTERVALLUM = 0) then
suspend;
else
    if (P_INTERVALLUM = R_INTERVALLUM) then
suspend;
end
end
if (:P_UGYFELBESOROLAS = -1) then
...
R_BESOROLASERTEK = 0;
R_BESOROLASNEV = '';
for select UB.R_UGYFEL, UB.R_FIZDATUM, UB.R_BIZSZAM, UB.R_IKTATOSZAM, UB.R_TARTOSSZEG, UB.R_KOVOSSZEG
...
TIPUS = 2
into :R_INTERVALLUM;
if (R_INTERVALLUM is not null) then
begin
if (P_INTERVALLUM = 0) then
suspend;
else
